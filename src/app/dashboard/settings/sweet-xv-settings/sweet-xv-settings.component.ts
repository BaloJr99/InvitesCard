...
import { ISettingType } from 'src/app/core/models/enum';
...
export class SweetXvSettingsComponent implements AfterViewInit {
  ...
  @Input() set eventSelected(eventId: string) {
    this.sweetXvSettings = {
      eventId,
      isNew: true,
      settingType: ISettingType.SweetXv,
    } as ISettingAction;
  ...
  updateEventSettings() {
    ...
    if (this.eventSelected) {
      this.settingsService
        .updateEventSettings(this.formatEventSetting(), this.sweetXvSettings.eventId)
        .subscribe({
          next: (response: IMessageResponse) => {
            this.toastr.success(response.message);
          },
